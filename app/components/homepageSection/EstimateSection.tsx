...
const EstimateSection: React.FC = () => {
  ...
  const filterdPhone = phones
    .filter((p) => models.find((m) => m.model_name === selectedModel)?.id === p.model_id)
    .filter((p) => capacity.find((c) => c.id === p.capacity_id)?.capacity_value === selectedCapacity)
    .map((p) => p.id)
  ...
  return (
    <div
      ...
      <div className="flex w-full items-center justify-center p-4 md:justify-start">
        <div className="flex h-96 w-96 flex-col gap-4 rounded-md bg-white p-8 md:-mb-20">
          ...
          <div className="flex w-full flex-col gap-4">
            ...
            <DropdownSearch
              label="Storage"
              options={filteredCapacity.map((c) => ({ id: c.id, name: c.name || '' }))}
              selected={selectedCapacity}
              setSelected={(name) => setSelectedCapacity(name)}
            />
            <button
              onClick={() => router.push(`/detail/${filterdPhone[0]}`)}
